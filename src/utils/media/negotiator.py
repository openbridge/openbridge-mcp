...
class ResourceTypeNegotiator:
    ...
    def register_negotiator(
        ...
        logger.debug(
            "Registered negotiator for resource type: %s", resource_type
        )
    ...
    def negotiate(
        ...
        if resource_type and resource_type in self._negotiators:
            try:
                result = self._negotiators[resource_type](
                    method, url, available_types
                )
                if result:
                    logger.debug(
                        "Negotiated %s for %s resource", result, resource_type
                    )
                    return result
            except Exception as e:
                logger.warning(
                    "Negotiator failed for %s: %s", resource_type, e
                )
        return None
