...
class TokenKey:
    ...
    identity_id: Optional[str] = None          # e.g., "default", "direct-auth", remote identity ID
...
class PersistentTokenStore(InMemoryTokenStore):
        ...
        **kwargs
        ...
        if storage_path is None:
            ...
            if cache_dir:
                ...
                logger.info(f"Using cache directory from AMAZON_ADS_CACHE_DIR: {storage_path}")
        ...
        try:
            ...
            logger.warning(f"Could not set restricted permissions on {self._storage_path.parent}")
    ...
    def _encrypt_data(self, data: dict) -> dict:
        ...
        # Simplified - in production use cryptography.fernet or similar
        # This is a placeholder showing where encryption would happen
        logger.warning("Token encryption not implemented - storing in plaintext")
        return data
...
def create_token_store(persist: bool = True, **kwargs) -> TokenStore:
    ...
    Args:
        persist: Whether to enable persistent storage (default: True)
    ...
    # Default is True, but can be disabled by setting to "false"
    env_persist = os.getenv("AMAZON_ADS_TOKEN_PERSIST", "true").lower()
