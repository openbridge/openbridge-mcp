...
jwt_token_var: ContextVar[Optional[str]] = ContextVar(
    "jwt_token", default=None
)
jwt_claims_var: ContextVar[Optional[dict]] = ContextVar(
    "jwt_claims", default=None
)
...
class AuthConfig:
    ...
    def __init__(self):
        ...
        self.refresh_token_request_builder: Optional[Callable[[str], dict]] = (
            None
        )
        ...
        # Caching settings
        self.jwt_cache_ttl = (
            3000  # 50 minutes (OpenBridge JWTs typically last 1 hour)
        )
    ...
    def load_from_env(self) -> None:
        ...
        self.jwt_verify_iss = (
            os.getenv("JWT_VERIFY_ISS", "true").lower() == "true"
        )
        self.jwt_verify_aud = (
            os.getenv("JWT_VERIFY_AUD", "true").lower() == "true"
        )
        ...
        if cache_ttl:
            try:
                ...
                logger.warning(
                    f"Invalid JWT_CACHE_TTL: {cache_ttl}, using default"
                )
    ...
    def validate(self) -> bool:
        ...
        if self.refresh_token_enabled:
            if not self.refresh_token_endpoint:
                logger.error(
                    "Refresh token enabled but no endpoint configured"
                )
                return False
...
class JWTCache:
    ...
    def __init__(self, ttl: int = 3000, cleanup_interval: int = 300, auth_manager=None):
        ...
        self._auth_manager = auth_manager  # Optional AuthManager for TokenStore integration
    ...
    def get(self, key: str) -> Optional[str]:
        ...
        if self._auth_manager and hasattr(self._auth_manager, 'get_token'):
            ...
            if len(parts) >= 2:
                ...
                try:
                    # Get from unified token store
                    loop = asyncio.get_event_loop()
                    token_entry = loop.run_until_complete(
                        self._auth_manager.get_token(
                    ...
                    logger.debug(f"Failed to get token from TokenStore: {e}")
        ...
        with self._lock:
            if key in self._cache:
                ...
                if now < expiry_time:
                    ...
                    del self._cache[key]
                    logger.debug(
                        f"Removed expired cache entry for key: {key[:20]}..."
                    )
    ...
    def _cleanup(self, now: float) -> None:
        ...
        with self._lock:
            ...
            if expired_keys:
                logger.info(
                    f"Cleaned up {len(expired_keys)} expired JWT cache entries"
                )
...
class RefreshTokenMiddleware(Middleware):
    ...
    async def on_request(self, context: MiddlewareContext, call_next):
        ...
        try:
            ...
            if (
                self.config.refresh_token_pattern
                and self.config.refresh_token_pattern(token)
            ):
                self.logger.debug(
                    "Detected refresh token format, checking cache..."
                )
    ...
    async def _get_cached_or_convert_jwt(
    ...
    async def _convert_refresh_to_jwt(
        self, refresh_token: str
    ) -> Optional[str]:
        ...
        try:
            ...
            jwt_token = self.config.refresh_token_response_parser(
                response_data
            )

            if jwt_token:
                self.logger.debug(
                    "Successfully converted refresh token to JWT"
                )
                return jwt_token
            else:
...
class JWTAuthenticationMiddleware(Middleware):
    ...
    def __init__(self, config: AuthConfig):
        ...
        self.logger = logging.getLogger(
            f"{__name__}.JWTAuthenticationMiddleware"
        )
    ...
    async def on_request(self, context: MiddlewareContext, call_next):
        ...
        try:
            ...
            if jwt_token:
                ...
                self.logger.debug(
                    f"Extracted token (length: {len(token)}, type: JWT)"
                )
    ...
    async def _validate_jwt_without_signature(
        self, token: str
    ) -> Optional[dict]:
        ...
        try:
            ...
            if " " in token:
                self.logger.error(
                    "Token contains whitespace - may be corrupted"
                )
                self.logger.error(
            ...
            if expires_at:
                try:
                    exp_time = datetime.fromtimestamp(float(expires_at), tz=timezone.utc)
                    if datetime.now(timezone.utc) > exp_time:
                        self.logger.warning("JWT token is expired")
                        return None
                except (ValueError, TypeError):
                    self.logger.warning(
                        f"Invalid expires_at format: {expires_at}"
                    )
                    return None
            ...
            if "Invalid header padding" in str(e):
                ...
                try:
                    ...
                    if len(parts) >= 1:
                        self.logger.error(
                            f"Header part: {sanitize_string(parts[0])}"
                        )
                        if len(parts) >= 2:
                            self.logger.error(
        ...
        if self.config.jwt_jwks_uri:
            try:
                ...
                response = await client.get(
                    self.config.jwt_jwks_uri, timeout=10.0
                )
                response.raise_for_status()
                jwks = response.json()
    ...
    def _jwk_to_pem(self, jwk_key: dict) -> Optional[str]:
        ...
        try:
            ...
            if jwk_key.get("kty") == "RSA":
                ...
                n = int.from_bytes(
                    jwt.utils.base64url_decode(jwk_key["n"]), "big"
                )
                e = int.from_bytes(
                    jwt.utils.base64url_decode(jwk_key["e"]), "big"
                )
...
def get_current_jwt() -> Optional[str]:
    ...
    Returns:
        The JWT token for the current request context, or None if not available.
...
def get_current_claims() -> Optional[dict]:
    ...
    Returns:
        The JWT claims dictionary for the current request context, or None if not available.
...
def create_auth_middleware(
    ...
    Args:
        config: Optional AuthConfig instance. If None, creates a new
            config and loads settings from environment variables.
        refresh_token_middleware: Whether to include refresh token
            conversion middleware (default: True).
        jwt_middleware: Whether to include JWT validation middleware
            (default: True).
        auth_manager: Optional AuthManager instance for token store integration.

    Returns:
        A list of middleware instances ready for use with FastMCP.
        ...
        if config.refresh_token_enabled and config.refresh_token_endpoint:
            if "openbridge.io" in config.refresh_token_endpoint:
                ...
                config = create_json_api_refresh_token_config(
                    ...
                    token_type_name=os.getenv(
                        "JSON_API_TOKEN_TYPE_NAME", "APIAuth"
                    ),
...
def create_json_api_refresh_token_config(
    ...
    Args:
        endpoint_url: The refresh token endpoint URL.
        token_type_name: The JSON:API resource type name for tokens.
        required_claims: List of required JWT claims for validation.
        verify_signature: Whether to verify JWT signatures (default: True).

    Returns:
        A configured AuthConfig instance for JSON:API refresh tokens.
...
def create_openbridge_config() -> AuthConfig:
    ...
    Returns:
        A configured AuthConfig instance for OpenBridge authentication.
    ...
    endpoint_url = os.getenv(
        "REFRESH_TOKEN_ENDPOINT", f"{auth_base}/auth/api/ref"
    )
...
def create_auth0_config(domain: str, audience: str) -> AuthConfig:
    ...
    Args:
        domain: The Auth0 domain (e.g., "example.auth0.com").
        audience: The Auth0 API audience identifier.

    Returns:
        A configured AuthConfig instance for Auth0 authentication.
...
async def get_auth_info() -> Dict[str, Any]:
    ...
    Returns:
        A dictionary containing authentication configuration information
        with keys:
