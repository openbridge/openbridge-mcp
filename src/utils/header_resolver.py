...
class HeaderNameResolver:
    ...
    _SCOPE_PAT = re.compile(
        r"(amazon[- ]advertising[- ]api[- ]scope|scope)$", re.I
    )
    _ACCOUNT_PAT = re.compile(
        r"(amazon[- ]ads[- ]accountid|account[-_ ]id)$", re.I
    )
    ...
    @staticmethod
    def _prefer(
        names: Iterable[str], fallbacks: Iterable[str]
    ) -> Optional[str]:
        """
        ...
        if discovered:
            ...
            aa = [
                n
                for n in discovered
                if n.lower().startswith("amazon-advertising-api-")
            ]
    ...
    def prefer_scope(self) -> Optional[str]:
        ...
        return self._prefer(
            self.scope_header_names, ["Amazon-Advertising-API-Scope"]
        )

    def prefer_account(self) -> Optional[str]:
        """Get the preferred account ID header name."""
        return self._prefer(
            self.account_header_names, ["Amazon-Ads-AccountId"]
        )
