...
class HTTPClientManager:
    ...
    async def get_client(
        ...
        **kwargs,
        ...
        HTTP version, and redirect following settings.
        ...
        if http2_flag is None:
            http2_flag = (
                os.getenv("HTTP_ENABLE_HTTP2", "false").lower() == "true"
            )
        if http2_flag:
            try:
        ...
        cache_key = str(
            (base_url or "default", t_key, l_key, http2_flag, follow)
        )
        ...
        if cache_key not in self._clients:
            async with self._lock:
                if cache_key not in self._clients:
                    ...
                    logger.debug("Created new HTTP client for %s", cache_key)
    ...
    async def close_all(self):
        ...
        if self._is_closing:
            logger.debug(
                "Already closing HTTP clients, skipping duplicate call"
            )
            return
...
async def get_http_client(**kwargs) -> httpx.AsyncClient:
    ...
    manager to retrieve or create an HTTP client.
