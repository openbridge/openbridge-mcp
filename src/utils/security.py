        ...
        try:
            ...
            if hasattr(record, "args") and record.args:
                ...
                try:
                    ...
                    record.args = (
                        None  # Clear args since we've already formatted
                    )
...
def setup_secure_logging(level: str = "INFO") -> None:
    ...
    logging.basicConfig(
        level=getattr(logging, level.upper()), handlers=[handler]
    )
...
def sanitize_html_input(
    value: str, allowed_tags: Optional[List[str]] = None
) -> str:
...
def sanitize_filename(filename: str) -> str:
    ...
    if len(filename) > max_length:
        ...
        filename = (
            f"{name[:max_length-len(ext)-1]}.{ext}"
            if ext
...
def validate_url(url: str, allowed_schemes: Optional[List[str]] = None) -> str:
    ...
    if url.lower().startswith(("javascript:", "data:", "vbscript:")):
        raise ValidationError(
            "Invalid URL: potentially dangerous scheme", field="url"
        )
